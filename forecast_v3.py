...
print(f"Current: σ_stat={sc_stat:.4f}, σ_sys={sc_sys:.4f}, σ_tot={sc_005:.4f}")
...
for name, nl, pz, sq, ref in surveys:
    # Statistical: ∝ 1/√N_lens
    sc_stat_f = sc_stat / np.sqrt(nl)
    # Systematic: photo-z improvement × shear quality
    sc_sys_f = sc_sys * pz * sq
...
ax=axes[1]
# Show asymptote: as N_lens → ∞, σ_c → σ_sys
nl_grid=np.logspace(0,4,50)
sc_asymp=np.sqrt((sc_stat/np.sqrt(nl_grid))**2 + sc_sys**2)
ax.loglog(nl_grid,sc_asymp,"b-",lw=2)
# Mark surveys
for name,nl,_,_,_ in surveys:
    sc=np.sqrt((sc_stat/np.sqrt(nl))**2 + sc_sys**2)
...
ax.set_title("(b) Asymptote: σ_c → σ_sys at large N")
