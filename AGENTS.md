...
- P17 Pulsar Glitch Size Distribution: Phases 0-3 COMPLETE. Novel finding: glitch sizes follow Weibull (stretched exponential) distribution P(>Δν) = exp(-(Δν/λ)^k) with k≈0.35, beating power law by ΔAIC=11917 and lognormal by ΔAIC=12.8. Truncation test shows k varies with threshold (0.35→0.80), suggesting possible multi-scale process. Breakpoint analysis confirms two populations split at ~5×10⁻⁸: small glitches (k≈0.65, consistent with crust cracking) and large glitches (k≈0.75-1.0, near-exponential, possibly superfluid unpinning). Per-pulsar k varies from 0.58 (Crab) to 1.39 (J0537-6910). Truncated single-Weibull model (same k, different λ) is competitive with two-population model (ΔAIC=-8.8), so the simpler interpretation is one process with pulsar-dependent characteristic scale.
...
**Pulsar glitch sizes follow a Weibull (stretched exponential) distribution:** P(>Δν) = exp(-(Δν/λ)^k) with k≈0.35, λ≈2.7×10⁻⁷ (MLE). Beats power law by ΔAIC=11917 and lognormal by ΔAIC=12.8. Truncation test shows k varies with threshold (0.35→0.80), suggesting possible multi-scale process. Breakpoint analysis confirms two populations split at ~5×10⁻⁸: small glitches (k≈0.65, consistent with crust cracking) and large glitches (k≈0.75-1.0, near-exponential, possibly superfluid unpinning). Per-pulsar k varies from 0.58 (Crab) to 1.39 (J0537-6910). Truncated single-Weibull model (same k, different λ) is competitive with two-population model (ΔAIC=-8.8), so the simpler interpretation is one process with pulsar-dependent characteristic scale.
